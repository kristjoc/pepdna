...
static int pepdna_fwd_request(struct pepcon *con)
{
	struct msghdr msg = {
	...
	if (rc > 0) {
		con->rx_buff[rc] = '\0';
		parse_request(con->rx_buff, content, url);

		rc = pepdna_send_to_ccn_relay(con->rsock, url);
		request_done = true;
	}
...
void pepdna_udp_data_ready(struct sock *sk)
{
	...
	if (con) {
		get_con(con);
		if (!queue_work(con->srv->r2l_wq, &con->r2l_work)) {
...
void pepdna_con_c2i_work(struct work_struct *work)
{
	...
	while (lconnected(con)) {
		rc = pepdna_con_i2i_fwd(con, con->rsock, con->lsock);
		if (rc <= 0) {
...
void pepdna_con_i2c_work(struct work_struct *work)
{
	...
	while (lconnected(con)) {
		if (unlikely(!request_done)) {
			...
			rc = pepdna_con_i2i_fwd(con, con->lsock, con->rsock);
			if (rc <= 0) {
				if (rc == -EAGAIN) /* FIXME: Handle -EAGAIN flood */
					break;
				close_con(con);
